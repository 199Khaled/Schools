-- Stored Procedures for Table: StudentExamResults

Use [SchoolsDb];
Go


CREATE OR ALTER PROCEDURE SP_Get_StudentExamResults_ByID
(
    @ResultID int
)
AS
...
CREATE OR ALTER PROCEDURE SP_Get_All_StudentExamResults
AS
BEGIN
    BEGIN TRY
...
CREATE OR ALTER PROCEDURE SP_Add_StudentExamResults
(
    @StudentID int,
    @ExamID int,
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@StudentID)) IS NULL OR LTRIM(RTRIM(@ExamID)) IS NULL
        BEGIN
        ...
        INSERT INTO StudentExamResults ([StudentID],[ExamID],[Score])
...
CREATE OR ALTER PROCEDURE SP_Update_StudentExamResults_ByID
(
    @ResultID int,
    @StudentID int,
    @ExamID int,
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@StudentID)) IS NULL OR LTRIM(RTRIM(@StudentID)) = '' OR LTRIM(RTRIM(@ExamID)) IS NULL OR LTRIM(RTRIM(@ExamID)) = ''
        BEGIN
            RAISERROR('One or more required parameters are NULL or have only whitespace.', 16, 1);
            RETURN;
        END
...
CREATE OR ALTER PROCEDURE SP_Delete_StudentExamResults_ByID
(
    @ResultID int
)
AS
BEGIN
    ...
    BEGIN TRY
        ...
        IF NOT EXISTS (SELECT 1 FROM StudentExamResults WHERE ResultID = @ResultID)
        BEGIN
            EXEC SP_HandleError;
            RETURN;
        END
        ...
        DELETE FROM StudentExamResults WHERE ResultID = @ResultID;
...
CREATE OR ALTER PROCEDURE SP_Search_StudentExamResults_ByColumn
(
...
BEGIN
    BEGIN TRY
        ...
        SET @ActualColumn = 
            CASE @ColumnName
                WHEN 'ResultID' THEN 'ResultID'
        WHEN 'StudentID' THEN 'StudentID'
        WHEN 'ExamID' THEN 'ExamID'
        ...
        SET @SQL = N'SELECT * FROM ' + QUOTENAME('StudentExamResults') + 
