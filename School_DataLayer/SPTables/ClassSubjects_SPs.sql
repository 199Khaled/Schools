-- Stored Procedures for Table: ClassSubjects

Use [SchoolsDb];
Go


CREATE OR ALTER PROCEDURE SP_Get_ClassSubjects_ByID
(
    @ClassSubjectID int
)
AS
...
CREATE OR ALTER PROCEDURE SP_Get_All_ClassSubjects
AS
BEGIN
    BEGIN TRY
...
CREATE OR ALTER PROCEDURE SP_Add_ClassSubjects
(
    @ClassID int,
    @SubjectID int,
    @TeacherID int,
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@ClassID)) IS NULL OR LTRIM(RTRIM(@SubjectID)) IS NULL OR LTRIM(RTRIM(@TeacherID)) IS NULL
        BEGIN
        ...
        VALUES (    LTRIM(RTRIM(@ClassID)),
    LTRIM(RTRIM(@SubjectID)),
    LTRIM(RTRIM(@TeacherID)));
...
CREATE OR ALTER PROCEDURE SP_Update_ClassSubjects_ByID
(
    @ClassSubjectID int,
    @ClassID int,
    @SubjectID int,
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@ClassID)) IS NULL OR LTRIM(RTRIM(@ClassID)) = '' OR LTRIM(RTRIM(@SubjectID)) IS NULL OR LTRIM(RTRIM(@SubjectID)) = '' OR LTRIM(RTRIM(@TeacherID)) IS NULL OR LTRIM(RTRIM(@TeacherID)) = ''
        BEGIN
            RAISERROR('One or more required parameters are NULL or have only whitespace.', 16, 1);
            RETURN;
        END
        ...
        SET     [ClassID] = LTRIM(RTRIM(@ClassID)),
    [SubjectID] = LTRIM(RTRIM(@SubjectID)),
    [TeacherID] = LTRIM(RTRIM(@TeacherID))
        WHERE ClassSubjectID = @ClassSubjectID;
...
CREATE OR ALTER PROCEDURE SP_Delete_ClassSubjects_ByID
(
    @ClassSubjectID int
)
AS
BEGIN
    ...
    BEGIN TRY
        ...
        IF NOT EXISTS (SELECT 1 FROM ClassSubjects WHERE ClassSubjectID = @ClassSubjectID)
        BEGIN
            EXEC SP_HandleError;
            RETURN;
        END
...
CREATE OR ALTER PROCEDURE SP_Search_ClassSubjects_ByColumn
(
...
BEGIN
    BEGIN TRY
        ...
        SET @SQL = N'SELECT * FROM ' + QUOTENAME('ClassSubjects') + 
