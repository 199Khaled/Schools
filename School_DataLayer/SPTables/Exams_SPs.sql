-- Stored Procedures for Table: Exams

Use [SchoolsDb];
Go


CREATE OR ALTER PROCEDURE SP_Get_Exams_ByID
(
    @ExamID int
)
AS
...
CREATE OR ALTER PROCEDURE SP_Get_All_Exams
AS
BEGIN
    BEGIN TRY
...
CREATE OR ALTER PROCEDURE SP_Add_Exams
(
    @SubjectID int,
    @ClassID int,
    @ExamDate date,
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@SubjectID)) IS NULL OR LTRIM(RTRIM(@ClassID)) IS NULL OR LTRIM(RTRIM(@ExamDate)) IS NULL
        BEGIN
        ...
        INSERT INTO Exams ([SubjectID],[ClassID],[ExamDate])
        VALUES (    LTRIM(RTRIM(@SubjectID)),
    LTRIM(RTRIM(@ClassID)),
    LTRIM(RTRIM(@ExamDate)));
...
CREATE OR ALTER PROCEDURE SP_Update_Exams_ByID
(
    @ExamID int,
    @SubjectID int,
    @ClassID int,
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@SubjectID)) IS NULL OR LTRIM(RTRIM(@SubjectID)) = '' OR LTRIM(RTRIM(@ClassID)) IS NULL OR LTRIM(RTRIM(@ClassID)) = '' OR LTRIM(RTRIM(@ExamDate)) IS NULL OR LTRIM(RTRIM(@ExamDate)) = ''
        BEGIN
            RAISERROR('One or more required parameters are NULL or have only whitespace.', 16, 1);
            RETURN;
        END
...
CREATE OR ALTER PROCEDURE SP_Delete_Exams_ByID
(
    @ExamID int
)
AS
...
CREATE OR ALTER PROCEDURE SP_Search_Exams_ByColumn
(
...
BEGIN
    BEGIN TRY
        ...
        SET @SQL = N'SELECT * FROM ' + QUOTENAME('Exams') + 
