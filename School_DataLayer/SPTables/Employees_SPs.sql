-- Stored Procedures for Table: Employees

Use [SchoolsDb];
Go


CREATE OR ALTER PROCEDURE SP_Get_Employees_ByID
(
    @EmployeeID int
)
AS
...
CREATE OR ALTER PROCEDURE SP_Add_Employees
(
    @PersonID int,
    @Typ nvarchar(100),
    @HireDate date,
    @FireDate nvarchar(15) = NULL,
    @isAktive bit = NULL,
    @NewID INT OUTPUT
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@PersonID)) IS NULL OR LTRIM(RTRIM(@Typ)) IS NULL OR LTRIM(RTRIM(@HireDate)) IS NULL
        BEGIN
        ...
        INSERT INTO Employees ([PersonID],[Typ],[HireDate],[FireDate],[isAktive])
        VALUES (    LTRIM(RTRIM(@PersonID)),
    LTRIM(RTRIM(@Typ)),
    LTRIM(RTRIM(@HireDate)),
    LTRIM(RTRIM(@FireDate)),
    LTRIM(RTRIM(@isAktive))
);
...
CREATE OR ALTER PROCEDURE SP_Update_Employees_ByID
(
    @EmployeeID int,
    @PersonID int,
    @Typ nvarchar(100),
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@PersonID)) IS NULL OR LTRIM(RTRIM(@PersonID)) = '' OR LTRIM(RTRIM(@Typ)) IS NULL OR LTRIM(RTRIM(@Typ)) = '' OR LTRIM(RTRIM(@HireDate)) IS NULL OR LTRIM(RTRIM(@HireDate)) = ''
        BEGIN
            RAISERROR('One or more required parameters are NULL or have only whitespace.', 16, 1);
            RETURN;
        END
    ...
    [FireDate] = LTRIM(RTRIM(@FireDate)),
    [isAktive] = LTRIM(RTRIM(@isAktive))
...
CREATE OR ALTER PROCEDURE SP_Delete_Employees_ByID
(
    @EmployeeID int
)
AS
...
CREATE OR ALTER PROCEDURE SP_Search_Employees_ByColumn
(
...
BEGIN
    BEGIN TRY
        ...
        WHEN 'FireDate' THEN 'FireDate'
        WHEN 'isAktive' THEN 'isAktive'
                ELSE NULL
            END;
        ...
        SET @SQL = N'SELECT * FROM ' + QUOTENAME('Employees') + 
