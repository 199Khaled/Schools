-- Stored Procedures for Table: Payments

Use [SchoolsDb];
Go


CREATE OR ALTER PROCEDURE SP_Get_Payments_ByID
(
    @PaymentID int
)
AS
...
CREATE OR ALTER PROCEDURE SP_Get_All_Payments
AS
BEGIN
    BEGIN TRY
...
CREATE OR ALTER PROCEDURE SP_Add_Payments
(
    @StudentID int,
    @Amount decimal(10,2),
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@StudentID)) IS NULL OR LTRIM(RTRIM(@Amount)) IS NULL
        BEGIN
...
CREATE OR ALTER PROCEDURE SP_Update_Payments_ByID
(
    @PaymentID int,
    @StudentID int,
    @Amount decimal(10,2),
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@StudentID)) IS NULL OR LTRIM(RTRIM(@StudentID)) = '' OR LTRIM(RTRIM(@Amount)) IS NULL OR LTRIM(RTRIM(@Amount)) = ''
        BEGIN
            RAISERROR('One or more required parameters are NULL or have only whitespace.', 16, 1);
            RETURN;
        END
    ...
    [PaymentDate] = LTRIM(RTRIM(@PaymentDate)),
...
CREATE OR ALTER PROCEDURE SP_Delete_Payments_ByID
(
    @PaymentID int
)
AS
...
CREATE OR ALTER PROCEDURE SP_Search_Payments_ByColumn
(
...
BEGIN
    BEGIN TRY
        ...
        SET @SQL = N'SELECT * FROM ' + QUOTENAME('Payments') + 
