-- Stored Procedures for Table: الفصول_الدراسية

Use [SchoolsDb];
Go


CREATE OR ALTER PROCEDURE SP_Get_الفصول_الدراسية_ByID
(
    @معرّف_الفصل int
)
AS
...
CREATE OR ALTER PROCEDURE SP_Get_All_الفصول_الدراسية
AS
BEGIN
    BEGIN TRY
...
CREATE OR ALTER PROCEDURE SP_Add_الفصول_الدراسية
(
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@اسم_الفصل)) IS NULL OR LTRIM(RTRIM(@السعة)) IS NULL
        BEGIN
...
CREATE OR ALTER PROCEDURE SP_Update_الفصول_الدراسية_ByID
(
    @معرّف_الفصل int,
    @اسم_الفصل varchar(50),
    @السعة int,
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@اسم_الفصل)) IS NULL OR LTRIM(RTRIM(@اسم_الفصل)) = '' OR LTRIM(RTRIM(@السعة)) IS NULL OR LTRIM(RTRIM(@السعة)) = ''
        BEGIN
            RAISERROR('One or more required parameters are NULL or have only whitespace.', 16, 1);
            RETURN;
        END
...
CREATE OR ALTER PROCEDURE SP_Delete_الفصول_الدراسية_ByID
(
    @معرّف_الفصل int
)
AS
BEGIN
    ...
    BEGIN TRY
        ...
        IF NOT EXISTS (SELECT 1 FROM الفصول_الدراسية WHERE معرّف_الفصل = @معرّف_الفصل)
        BEGIN
            EXEC SP_HandleError;
            RETURN;
        END
        ...
        DELETE FROM الفصول_الدراسية WHERE معرّف_الفصل = @معرّف_الفصل;
...
CREATE OR ALTER PROCEDURE SP_Search_الفصول_الدراسية_ByColumn
(
...
BEGIN
    BEGIN TRY
        ...
        WHEN 'اسم_الفصل' THEN 'اسم_الفصل'
        WHEN 'السعة' THEN 'السعة'
        WHEN 'المبنى' THEN 'المبنى'
                ELSE NULL
            END;
        ...
        SET @SQL = N'SELECT * FROM ' + QUOTENAME('الفصول_الدراسية') + 
