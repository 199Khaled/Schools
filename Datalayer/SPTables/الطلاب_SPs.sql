-- Stored Procedures for Table: الطلاب

Use [SchoolsDb];
Go


CREATE OR ALTER PROCEDURE SP_Get_الطلاب_ByID
(
    @معرّف_الطالب int
)
AS
BEGIN
    BEGIN TRY
        ...
        SELECT *
        FROM الطلاب
        WHERE معرّف_الطالب = @معرّف_الطالب;
    END TRY
    BEGIN CATCH
...
CREATE OR ALTER PROCEDURE SP_Get_All_الطلاب
AS
BEGIN
    BEGIN TRY
...
CREATE OR ALTER PROCEDURE SP_Add_الطلاب
(
    @معرّف_الشخص int,
    @تاريخ_الالتحاق date = NULL,
    @NewID INT OUTPUT
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@معرّف_الشخص)) IS NULL
        BEGIN
        ...
        INSERT INTO الطلاب ([معرّف_الشخص],[تاريخ_الالتحاق])
        VALUES (    LTRIM(RTRIM(@معرّف_الشخص)),
    LTRIM(RTRIM(@تاريخ_الالتحاق)));
...
CREATE OR ALTER PROCEDURE SP_Update_الطلاب_ByID
(
    @معرّف_الطالب int,
    @معرّف_الشخص int,
    @تاريخ_الالتحاق date = NULL
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@معرّف_الشخص)) IS NULL OR LTRIM(RTRIM(@معرّف_الشخص)) = ''
        BEGIN
            RAISERROR('One or more required parameters are NULL or have only whitespace.', 16, 1);
            RETURN;
        END
    ...
    [تاريخ_الالتحاق] = LTRIM(RTRIM(@تاريخ_الالتحاق))
        WHERE معرّف_الطالب = @معرّف_الطالب;
...
CREATE OR ALTER PROCEDURE SP_Delete_الطلاب_ByID
(
    @معرّف_الطالب int
)
AS
BEGIN
    ...
    BEGIN TRY
        ...
        IF NOT EXISTS (SELECT 1 FROM الطلاب WHERE معرّف_الطالب = @معرّف_الطالب)
        BEGIN
            EXEC SP_HandleError;
            RETURN;
        END
        ...
        DELETE FROM الطلاب WHERE معرّف_الطالب = @معرّف_الطالب;
...
CREATE OR ALTER PROCEDURE SP_Search_الطلاب_ByColumn
(
...
BEGIN
    BEGIN TRY
        ...
        WHEN 'تاريخ_الالتحاق' THEN 'تاريخ_الالتحاق'
                ELSE NULL
            END;
        ...
        SET @SQL = N'SELECT * FROM ' + QUOTENAME('الطلاب') + 
