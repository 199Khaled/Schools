-- Stored Procedures for Table: الصفوف

Use [SchoolsDb];
Go


CREATE OR ALTER PROCEDURE SP_Get_الصفوف_ByID
(
    @معرّف_الصف int
)
AS
...
CREATE OR ALTER PROCEDURE SP_Get_All_الصفوف
AS
BEGIN
    BEGIN TRY
...
CREATE OR ALTER PROCEDURE SP_Add_الصفوف
(
    @اسم_الصف nvarchar(50),
    @المستوى_الصفّي nvarchar(50),
    @NewID INT OUTPUT
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@اسم_الصف)) IS NULL OR LTRIM(RTRIM(@المستوى_الصفّي)) IS NULL
        BEGIN
        ...
        INSERT INTO الصفوف ([اسم_الصف],[المستوى_الصفّي])
        VALUES (    LTRIM(RTRIM(@اسم_الصف)),
    LTRIM(RTRIM(@المستوى_الصفّي)));
...
CREATE OR ALTER PROCEDURE SP_Update_الصفوف_ByID
(
    @معرّف_الصف int,
    @اسم_الصف nvarchar(50),
    @المستوى_الصفّي nvarchar(50)
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@اسم_الصف)) IS NULL OR LTRIM(RTRIM(@اسم_الصف)) = '' OR LTRIM(RTRIM(@المستوى_الصفّي)) IS NULL OR LTRIM(RTRIM(@المستوى_الصفّي)) = ''
        BEGIN
            RAISERROR('One or more required parameters are NULL or have only whitespace.', 16, 1);
            RETURN;
        END
        ...
        UPDATE الصفوف
        SET     [اسم_الصف] = LTRIM(RTRIM(@اسم_الصف)),
    [المستوى_الصفّي] = LTRIM(RTRIM(@المستوى_الصفّي))
        WHERE معرّف_الصف = @معرّف_الصف;
...
CREATE OR ALTER PROCEDURE SP_Delete_الصفوف_ByID
(
    @معرّف_الصف int
)
AS
BEGIN
    ...
    BEGIN TRY
        ...
        IF NOT EXISTS (SELECT 1 FROM الصفوف WHERE معرّف_الصف = @معرّف_الصف)
        BEGIN
            EXEC SP_HandleError;
            RETURN;
        END
        ...
        DELETE FROM الصفوف WHERE معرّف_الصف = @معرّف_الصف;
...
CREATE OR ALTER PROCEDURE SP_Search_الصفوف_ByColumn
(
...
BEGIN
    BEGIN TRY
        ...
        WHEN 'اسم_الصف' THEN 'اسم_الصف'
        WHEN 'المستوى_الصفّي' THEN 'المستوى_الصفّي'
                ELSE NULL
            END;
        ...
        SET @SQL = N'SELECT * FROM ' + QUOTENAME('الصفوف') + 
