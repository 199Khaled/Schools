-- Stored Procedures for Table: مواد_المعلم

Use [SchoolsDb];
Go


CREATE OR ALTER PROCEDURE SP_Get_مواد_المعلم_ByID
(
...
CREATE OR ALTER PROCEDURE SP_Get_All_مواد_المعلم
AS
BEGIN
    BEGIN TRY
...
CREATE OR ALTER PROCEDURE SP_Add_مواد_المعلم
(
    @معرّف_المعلم int,
    @معرّف_المادة int,
    @NewID INT OUTPUT
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@معرّف_المعلم)) IS NULL OR LTRIM(RTRIM(@معرّف_المادة)) IS NULL
        BEGIN
...
CREATE OR ALTER PROCEDURE SP_Update_مواد_المعلم_ByID
(
    @معرّف_مادة_المعلم int,
    @معرّف_المعلم int,
    @معرّف_المادة int
...
BEGIN
    BEGIN TRY
        ...
        IF LTRIM(RTRIM(@معرّف_المعلم)) IS NULL OR LTRIM(RTRIM(@معرّف_المعلم)) = '' OR LTRIM(RTRIM(@معرّف_المادة)) IS NULL OR LTRIM(RTRIM(@معرّف_المادة)) = ''
        BEGIN
            RAISERROR('One or more required parameters are NULL or have only whitespace.', 16, 1);
            RETURN;
        END
...
CREATE OR ALTER PROCEDURE SP_Delete_مواد_المعلم_ByID
(
    @معرّف_مادة_المعلم int
)
AS
BEGIN
    ...
    BEGIN TRY
        ...
        IF NOT EXISTS (SELECT 1 FROM مواد_المعلم WHERE معرّف_مادة_المعلم = @معرّف_مادة_المعلم)
        BEGIN
            EXEC SP_HandleError;
            RETURN;
        END
        ...
        DELETE FROM مواد_المعلم WHERE معرّف_مادة_المعلم = @معرّف_مادة_المعلم;
...
CREATE OR ALTER PROCEDURE SP_Search_مواد_المعلم_ByColumn
(
...
BEGIN
    BEGIN TRY
        ...
        SET @ActualColumn = 
            CASE @ColumnName
                WHEN 'معرّف_مادة_المعلم' THEN 'معرّف_مادة_المعلم'
        ...
        SET @SQL = N'SELECT * FROM ' + QUOTENAME('مواد_المعلم') + 
